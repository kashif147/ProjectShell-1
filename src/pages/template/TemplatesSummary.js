...
import { getTemplates } from "../../features/template/GetTemplateSlice";
...
const TemplatesSummary = () => {
  ...
  const categoryFilterOptions = useMemo(() => {
    const options = buildAllTemplateCategoryFilterOptions(
      lookups,
      lookupsTypes,
    );
    return [{ value: "", label: "All Categories", key: "" }, ...options];
  ...
  const filteredData = useMemo(() => {
    return templates
      ...
      .filter((t) =>
        categoryFilter
          ? t.category?.toLowerCase() === String(categoryFilter).toLowerCase()
          : true,
      )
      .filter((t) =>
        statusFilter
  ...
  const handleEdit = (record) => {
    navigate(`/templateConfig?id=${encodeURIComponent(record?.key)}`);
  };
