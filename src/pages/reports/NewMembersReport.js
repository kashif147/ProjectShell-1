...
const NewMembersReport = () => {
    ...
    const columnDefs = [
        ...
        { headerName: "Category", field: "category", width: 150, sortable: true, filter: true },
        {
            headerName: "Status", field: "status", width: 120, sortable: true, filter: true,
            cellRenderer: (params) => <span style={{ color: "#52c41a", fontWeight: "bold" }}>{params.value}</span>
        },
    ];
    ...
    const fetchData = useCallback(async (currentPage, pageSize) => {
        ...
        try {
            ...
            const take = pageSize;
            const result = await reportService.getNewMembers(filters, skip, take);
    ...
    const handleExportPdf = async () => {
        try { setLoading(true); await reportService.exportPdf(filters, "new-members"); message.success("PDF export started"); } catch (error) { message.error(error.message || "Failed to export PDF"); } finally { setLoading(false); }
    };

    const handleExportExcel = async () => {
        try { setLoading(true); await reportService.exportExcel(filters, "new-members"); message.success("Excel export started"); } catch (error) { message.error(error.message || "Failed to export Excel"); } finally { setLoading(false); }
    };
                ...
                <Card>
                    <Row gutter={[16, 16]} style={{ marginBottom: "24px" }}>
                        ...
                        <Col xs={24} sm={12} md={6}>
                            ...
                            <Select style={{ width: "100%" }} placeholder="All Categories" allowClear onChange={(value) => handleFilterChange("category", value)}>
