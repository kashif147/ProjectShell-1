...
const LeaversReport = () => {
    ...
    const columnDefs = [
        ...
        {
            headerName: "Leaving Date", field: "leavingDate", width: 150, sortable: true, filter: true,
        ...
        { headerName: "Reason", field: "reason", width: 150, sortable: true, filter: true },
        {
            ...
            cellRenderer: (params) => <span style={{ color: "#ff4d4f", fontWeight: "bold" }}>{params.value}</span>
        },
    ];
    ...
    const fetchData = useCallback(async (currentPage, pageSize) => {
        ...
        try {
            ...
            const take = pageSize;
            const result = await reportService.getLeavers(filters, skip, take);
    ...
    const handleExportPdf = async () => {
        try { setLoading(true); await reportService.exportPdf(filters, "leavers"); message.success("PDF export started"); } catch (error) { message.error(error.message || "Failed to export PDF"); } finally { setLoading(false); }
    };

    const handleExportExcel = async () => {
        try { setLoading(true); await reportService.exportExcel(filters, "leavers"); message.success("Excel export started"); } catch (error) { message.error(error.message || "Failed to export Excel"); } finally { setLoading(false); }
    };
