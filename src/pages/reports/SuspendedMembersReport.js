...
const SuspendedMembersReport = () => {
    ...
    const columnDefs = [
        ...
        {
            headerName: "Suspended Date", field: "suspendedDate", width: 150, sortable: true, filter: true,
            valueFormatter: (params) => params.value ? dayjs(params.value).format("DD/MM/YYYY") : ""
        },
        { headerName: "Reason", field: "reason", width: 150, sortable: true, filter: true },
        {
            headerName: "Status", field: "status", width: 120, sortable: true, filter: true,
            cellRenderer: (params) => <span style={{ color: "#faad14", fontWeight: "bold" }}>{params.value}</span>
        },
    ];
    ...
    const fetchData = useCallback(async (currentPage, pageSize) => {
        ...
        try {
            ...
            const take = pageSize;
            const result = await reportService.getSuspendedMembers(filters, skip, take);
    ...
    const handleExportPdf = async () => {
        try { setLoading(true); await reportService.exportPdf(filters, "suspended-members"); message.success("PDF export started"); } catch (error) { message.error(error.message || "Failed to export PDF"); } finally { setLoading(false); }
    };

    const handleExportExcel = async () => {
        try { setLoading(true); await reportService.exportExcel(filters, "suspended-members"); message.success("Excel export started"); } catch (error) { message.error(error.message || "Failed to export Excel"); } finally { setLoading(false); }
    };
                ...
                <Card>
                    <Row gutter={[16, 16]} style={{ marginBottom: "24px" }}>
                        ...
                        <Col xs={24} sm={12} md={6}>
                            ...
                            <Select style={{ width: "100%" }} placeholder="All Reasons" allowClear onChange={(value) => handleFilterChange("reason", value)}>
...
export default SuspendedMembersReport;
