...
export function useUIInitialization(
  ...
  // Get detailed resource permissions with debouncing
  const [resourcePermissions, setResourcePermissions] = useState({});
  const [resourceLoading, setResourceLoading] = useState(false);
  const resourceLoadingRef = useRef(false);

  useEffect(() => {
    if (!token || !policyClient || !allLoaded || resourceLoadingRef.current)
      return;
    ...
    const timer = setTimeout(fetchResourcePermissions, 50);
    return () => clearTimeout(timer);
  }, [policyClient, token, allLoaded]);

  const uiState = useMemo(() => {
    const isLoading = permissionsLoading || resourceLoading || !capabilities;
    ...
    return {
      ...
      error,
      initialized: true,
      timestamp: Date.now(), // Use timestamp instead of ISO string for better performance
    };
  }, [
    permissionsLoading,
    resourceLoading,
