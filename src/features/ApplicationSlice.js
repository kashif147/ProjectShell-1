...
export const getAllApplications = createAsyncThunk(
  ...
  async (filters = {}, { rejectWithValue }) => {
    // ✅ Return early if filters is undefined - stop ALL execution
    if (filters === undefined) {
      console.log('⏹️ Filters is undefined - stopping all execution');
      ...
      if (Array.isArray(statusValues) && statusValues.length > 0) {
        ...
        if (operator === '!=') {
          ...
          if (includesDraft && filteredStatus.length === 0) {
            ...
            const apiApplications = response.data?.data?.applications || [];
