...
const ResignedMembersReport = () => {
    ...
    const columnDefs = [
        ...
        {
            headerName: "Resignation Date", field: "resignationDate", width: 150, sortable: true, filter: true,
        ...
        { headerName: "Reason", field: "reason", width: 150, sortable: true, filter: true },
        {
            ...
            cellRenderer: (params) => <span style={{ color: "#722ed1", fontWeight: "bold" }}>{params.value}</span>
        },
    ];
    ...
    const fetchData = useCallback(async (currentPage, pageSize) => {
        ...
        try {
            ...
            const take = pageSize;
            const result = await reportService.getResignedMembers(filters, skip, take);
    ...
    const handleExportPdf = async () => {
        try { setLoading(true); await reportService.exportPdf(filters, "resigned-members"); message.success("PDF export started"); } catch (error) { message.error(error.message || "Failed to export PDF"); } finally { setLoading(false); }
    };

    const handleExportExcel = async () => {
        try { setLoading(true); await reportService.exportExcel(filters, "resigned-members"); message.success("Excel export started"); } catch (error) { message.error(error.message || "Failed to export Excel"); } finally { setLoading(false); }
    };
...
export default ResignedMembersReport;
